...
function playerStateFromSteamId(steamId: string, nickname: string): PlayerState {
  return {
    input: steamId,
    steamId,
    nickname,
...
export default function EditTeamForm({ tournamentId, team, maxRankTier }: Props) {
  const router = useRouter();

  const [formError, setFormError] = useState<string | null>(null);
  const [submitting, setSubmitting] = useState(false);
  const [players, setPlayers] = useState<PlayerState[]>(() =>
    team.players.map((p) => playerStateFromSteamId(p.steamId, p.nickname))
  );
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!allResolved) {
      setFormError("All 5 players must have a valid Steam ID.");
      return;
    }
    ...
    try {
      const res = await fetch(`/api/tournaments/${tournamentId}/teams/${team.id}`, {
        method: "PUT",
    ...
    <div className="max-w-xl mx-auto p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mb-3">Player 1 will be set as the team captain. Enter a Steam ID to refresh player data.</p>
...
function PlayerCard({ index, isCaptain, player, maxRankTier, onSteamInputChange, onNicknameChange }: PlayerCardProps) {
  ...
  return (
    <div className={`border rounded-lg p-4 space-y-3 ${isCaptain ? "border-amber-700 bg-amber-950/30" : "border-gray-700"}`}>
      ...
      {/* Profile preview — shown when lookup resolves */}
      {player.profile && (
      ...
      {/* Nickname — always editable, pre-filled from existing data or lookup */}
      <div>
